...
namespace CP
{
  ...
  public:
    ...
    // copy constructor
    priority_queue(const priority_queue<T, C> &a) : mData(new T[a.mCap]()), mCap(a.mCap), mSize(a.mSize), mLess(a.mLess)
    {
    ...
    // default constructor
    priority_queue(const C &c = C()) : mData(new T[1]()),
                                       mCap(1),
                                       mSize(0),
                                       mLess(c)
    {
    ...
    priority_queue<T, C> &operator=(priority_queue<T, C> other)
    {
      using std::swap;
