    ...
    public:
        ...
        void reorder(int pos, std::vector<int> selected)
        {
            ...
            for (int i = 0, idx = 0; i <= size; ++i)
            {
                if (i == pos)
                    insert_point = it;
                if (idx < selected.size() && selected[idx] == i)
                {
                    if (!first)
                    {
                    ...
                    {
                        node *n = new node(*it, last, nullptr);
                        last->next = n;
                        last = n;
                    }
                    it = erase(it);
            ...
            first->prev = insert_point.ptr->prev;
            last->next = insert_point.ptr;
            insert_point.ptr->prev->next = first;
            insert_point.ptr->prev = last;
            mSize = size;
        }
    };
}
